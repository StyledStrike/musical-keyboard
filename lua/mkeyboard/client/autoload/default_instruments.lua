MKeyboard:RegisterInstrument( "Grand Piano", "sound/musical_keyboard/instruments/grand_piano", {
{ note = 23, fileName = "B0.ogg" },
{ note = 24, fileName = "C1.ogg" },
{ note = 25, fileName = "C^1.ogg" },
{ note = 28, fileName = "E1.ogg" },
{ note = 31, fileName = "G1.ogg" },
{ note = 35, fileName = "B1.ogg" },
{ note = 36, fileName = "C2.ogg" },
{ note = 37, fileName = "C^2.ogg" },
{ note = 40, fileName = "E2.ogg" },
{ note = 43, fileName = "G2.ogg" },
{ note = 47, fileName = "B2.ogg" },
{ note = 48, fileName = "C3.ogg" },
{ note = 49, fileName = "C^3.ogg" },
{ note = 52, fileName = "E3.ogg" },
{ note = 55, fileName = "G3.ogg" },
{ note = 59, fileName = "B3.ogg" },
{ note = 60, fileName = "C4.ogg" },
{ note = 61, fileName = "C^4.ogg" },
{ note = 64, fileName = "E4.ogg" },
{ note = 67, fileName = "G4.ogg" },
{ note = 71, fileName = "B4.ogg" },
{ note = 72, fileName = "C5.ogg" },
{ note = 73, fileName = "C^5.ogg" },
{ note = 76, fileName = "E5.ogg" },
{ note = 79, fileName = "G5.ogg" },
{ note = 83, fileName = "B5.ogg" },
{ note = 84, fileName = "C6.ogg" },
{ note = 85, fileName = "C^6.ogg" },
{ note = 88, fileName = "E6.ogg" },
{ note = 91, fileName = "G6.ogg" },
{ note = 95, fileName = "B6.ogg" },
{ note = 96, fileName = "C7.ogg" },
{ note = 97, fileName = "C^7.ogg" },
{ note = 100, fileName = "E7.ogg" },
{ note = 103, fileName = "G7.ogg" }
}, {
    volume = 0.5,
    releaseTime = 0.15
} )

MKeyboard:RegisterInstrument( "Grand Piano (Soft)", "sound/musical_keyboard/instruments/grand_piano_soft", {
{ note = 23, fileName = "B0.ogg" },
{ note = 24, fileName = "C1.ogg" },
{ note = 25, fileName = "C^1.ogg" },
{ note = 28, fileName = "E1.ogg" },
{ note = 31, fileName = "G1.ogg" },
{ note = 35, fileName = "B1.ogg" },
{ note = 36, fileName = "C2.ogg" },
{ note = 37, fileName = "C^2.ogg" },
{ note = 40, fileName = "E2.ogg" },
{ note = 43, fileName = "G2.ogg" },
{ note = 47, fileName = "B2.ogg" },
{ note = 48, fileName = "C3.ogg" },
{ note = 49, fileName = "C^3.ogg" },
{ note = 52, fileName = "E3.ogg" },
{ note = 55, fileName = "G3.ogg" },
{ note = 59, fileName = "B3.ogg" },
{ note = 60, fileName = "C4.ogg" },
{ note = 61, fileName = "C^4.ogg" },
{ note = 64, fileName = "E4.ogg" },
{ note = 67, fileName = "G4.ogg" },
{ note = 71, fileName = "B4.ogg" },
{ note = 72, fileName = "C5.ogg" },
{ note = 73, fileName = "C^5.ogg" },
{ note = 76, fileName = "E5.ogg" },
{ note = 79, fileName = "G5.ogg" },
{ note = 83, fileName = "B5.ogg" },
{ note = 84, fileName = "C6.ogg" },
{ note = 85, fileName = "C^6.ogg" },
{ note = 88, fileName = "E6.ogg" },
{ note = 91, fileName = "G6.ogg" },
{ note = 95, fileName = "B6.ogg" },
{ note = 96, fileName = "C7.ogg" },
{ note = 97, fileName = "C^7.ogg" },
{ note = 100, fileName = "E7.ogg" },
{ note = 103, fileName = "G7.ogg" }
}, {
    volume = 0.5,
    releaseTime = 0.2
} )

MKeyboard:RegisterInstrument( "Honky Tonk", "sound/musical_keyboard/instruments/honky_tonk", {
{ note = 24, fileName = "C1.ogg" },
{ note = 31, fileName = "G1.ogg" },
{ note = 36, fileName = "C2.ogg" },
{ note = 43, fileName = "G2.ogg" },
{ note = 48, fileName = "C3.ogg" },
{ note = 55, fileName = "G3.ogg" },
{ note = 60, fileName = "C4.ogg" },
{ note = 67, fileName = "G4.ogg" },
{ note = 72, fileName = "C5.ogg" },
{ note = 79, fileName = "G5.ogg" },
{ note = 84, fileName = "C6.ogg" },
{ note = 91, fileName = "G6.ogg" },
{ note = 96, fileName = "C7.ogg" },
{ note = 103, fileName = "G7.ogg" }
}, {
    volume = 0.28,
    releaseTime = 0.2
} )

MKeyboard:RegisterInstrument( "Electric Piano", "sound/musical_keyboard/instruments/electric_piano", {
{ note = 36, fileName = "C2.ogg",  loopStart = 2.659048, loopEnd = 3.516349 },
{ note = 40, fileName = "E2.ogg",  loopStart = 2.638186, loopEnd = 3.500884 },
{ note = 44, fileName = "G^2.ogg", loopStart = 2.626168, loopEnd = 3.397619 },
{ note = 48, fileName = "C3.ogg",  loopStart = 3.835465, loopEnd = 4.646780 },
{ note = 52, fileName = "E3.ogg",  loopStart = 2.771474, loopEnd = 3.628095 },
{ note = 56, fileName = "G^3.ogg", loopStart = 2.573129, loopEnd = 3.397619 },
{ note = 60, fileName = "C4.ogg",  loopStart = 2.321859, loopEnd = 3.156122 },
{ note = 64, fileName = "E4.ogg",  loopStart = 2.275896, loopEnd = 3.105147 },
{ note = 68, fileName = "G^4.ogg", loopStart = 2.561066, loopEnd = 3.397619 },
{ note = 72, fileName = "C5.ogg",  loopStart = 2.776916, loopEnd = 3.614989 },
{ note = 76, fileName = "E5.ogg",  loopStart = 2.275918, loopEnd = 3.105147 },
{ note = 80, fileName = "G^5.ogg", loopStart = 2.568299, loopEnd = 3.397619 },
{ note = 84, fileName = "C6.ogg",  loopStart = 2.413288, loopEnd = 3.246599 },
{ note = 88, fileName = "E6.ogg",  loopStart = 2.272132, loopEnd = 3.105147 },
{ note = 92, fileName = "G^6.ogg", loopStart = 2.564082, loopEnd = 3.397619 },
{ note = 96, fileName = "C7.ogg",  loopStart = 2.182857, loopEnd = 3.016168 }
}, {
    volume = 0.2
} )

MKeyboard:RegisterInstrument( "Analog Piano", "sound/musical_keyboard/instruments/analog_piano", {
{ note = 23, fileName = "B0.ogg" },
{ note = 24, fileName = "C1.ogg" },
{ note = 25, fileName = "C^1.ogg" },
{ note = 28, fileName = "E1.ogg" },
{ note = 31, fileName = "G1.ogg" },
{ note = 35, fileName = "B1.ogg" },
{ note = 36, fileName = "C2.ogg" },
{ note = 37, fileName = "C^2.ogg" },
{ note = 40, fileName = "E2.ogg" },
{ note = 43, fileName = "G2.ogg" },
{ note = 47, fileName = "B2.ogg" },
{ note = 48, fileName = "C3.ogg" },
{ note = 49, fileName = "C^3.ogg" },
{ note = 52, fileName = "E3.ogg" },
{ note = 55, fileName = "G3.ogg" },
{ note = 59, fileName = "B3.ogg" },
{ note = 60, fileName = "C4.ogg" },
{ note = 61, fileName = "C^4.ogg" },
{ note = 64, fileName = "E4.ogg" },
{ note = 67, fileName = "G4.ogg" },
{ note = 71, fileName = "B4.ogg" },
{ note = 72, fileName = "C5.ogg" },
{ note = 73, fileName = "C^5.ogg" },
{ note = 76, fileName = "E5.ogg" },
{ note = 79, fileName = "G5.ogg" },
{ note = 83, fileName = "B5.ogg" },
{ note = 84, fileName = "C6.ogg" },
{ note = 85, fileName = "C^6.ogg" },
{ note = 88, fileName = "E6.ogg" },
{ note = 91, fileName = "G6.ogg" },
{ note = 95, fileName = "B6.ogg" },
{ note = 96, fileName = "C7.ogg" },
{ note = 97, fileName = "C^7.ogg" },
{ note = 100, fileName = "E7.ogg" },
{ note = 103, fileName = "G7.ogg" }
}, {
    volume = 0.35,
    releaseTime = 0.3
} )

MKeyboard:RegisterInstrument( "Binary Piano", "sound/musical_keyboard/instruments/binary_piano", {
{ note = 23, fileName = "B0.ogg" },
{ note = 24, fileName = "C1.ogg" },
{ note = 28, fileName = "E1.ogg" },
{ note = 31, fileName = "G1.ogg" },
{ note = 35, fileName = "B1.ogg" },
{ note = 36, fileName = "C2.ogg" },
{ note = 40, fileName = "E2.ogg" },
{ note = 43, fileName = "G2.ogg" },
{ note = 47, fileName = "B2.ogg" },
{ note = 48, fileName = "C3.ogg" },
{ note = 52, fileName = "E3.ogg" },
{ note = 55, fileName = "G3.ogg" },
{ note = 59, fileName = "B3.ogg" },
{ note = 60, fileName = "C4.ogg" },
{ note = 64, fileName = "E4.ogg" },
{ note = 67, fileName = "G4.ogg" },
{ note = 71, fileName = "B4.ogg" },
{ note = 72, fileName = "C5.ogg" },
{ note = 76, fileName = "E5.ogg" },
{ note = 79, fileName = "G5.ogg" },
{ note = 83, fileName = "B5.ogg" },
{ note = 84, fileName = "C6.ogg" },
{ note = 88, fileName = "E6.ogg" },
{ note = 91, fileName = "G6.ogg" },
{ note = 95, fileName = "B6.ogg" },
{ note = 96, fileName = "C7.ogg" },
{ note = 100, fileName = "E7.ogg" },
{ note = 103, fileName = "G7.ogg" }
}, {
    volume = 0.4,
    releaseTime = 0.3
} )

MKeyboard:RegisterInstrument( "New Age", "sound/musical_keyboard/instruments/new_age", {
{ note = 36, fileName = "C2.ogg", loopStart = 1.911791, loopEnd = 4.069683 },
{ note = 48, fileName = "C3.ogg", loopStart = 1.621134, loopEnd = 4.185964 },
{ note = 60, fileName = "C4.ogg", loopStart = 2.051111, loopEnd = 3.959025 },
{ note = 72, fileName = "C5.ogg", loopStart = 1.395170, loopEnd = 4.170136 },
{ note = 84, fileName = "C6.ogg", loopStart = 1.777007, loopEnd = 3.984444 },
}, {
    volume = 0.3
} )

MKeyboard:RegisterInstrument( "Soft Synth", "sound/musical_keyboard/instruments/soft_synth", {
    { note = 24, fileName = "C1.ogg", loopStart = 4.363061, loopEnd = 13.699025 },
    { note = 36, fileName = "C2.ogg", loopStart = 2.374785, loopEnd = 10.100658 },
    { note = 48, fileName = "C3.ogg", loopStart = 2.071859, loopEnd = 11.399025 },
    { note = 60, fileName = "C4.ogg", loopStart = 1.317256, loopEnd = 8.178027 },
    { note = 72, fileName = "C5.ogg", loopStart = 3.478277, loopEnd = 8.009524 },
    { note = 84, fileName = "C6.ogg", loopStart = 1.611497, loopEnd = 5.428753 },
    { note = 96, fileName = "C7.ogg", loopStart = 1.984399, loopEnd = 5.960340 }
}, {
    volume = 0.45,
    releaseTime = 0.3
} )

MKeyboard:RegisterInstrument( "Polysynth", "sound/musical_keyboard/instruments/polysynth", {
{ note = 48, fileName = "C3.ogg", loopStart = 5.926327, loopEnd = 10.794785 },
{ note = 60, fileName = "C4.ogg", loopStart = 2.767959, loopEnd = 8.916122 },
{ note = 72, fileName = "C5.ogg", loopStart = 1.488141, loopEnd = 5.711837 },
{ note = 84, fileName = "C6.ogg", loopStart = 1.630204, loopEnd = 4.066236 },
{ note = 96, fileName = "C7.ogg", loopStart = 0.981066, loopEnd = 2.791406 }
}, {
    volume = 0.45,
    releaseTime = 0.3
} )

MKeyboard:RegisterInstrument( "Space Voice", "sound/musical_keyboard/instruments/space_voice", {
{ note = 36, fileName = "C2.ogg", loopStart = 8.013039, loopEnd = 14.327732 },
{ note = 48, fileName = "C3.ogg", loopStart = 4.014739, loopEnd = 10.326077 },
{ note = 60, fileName = "C4.ogg", loopStart = 3.592744, loopEnd = 6.742472 },
{ note = 72, fileName = "C5.ogg", loopStart = 1.792449, loopEnd = 4.148980 },
{ note = 84, fileName = "C6.ogg", loopStart = 1.215873, loopEnd = 3.039637 },
{ note = 96, fileName = "C7.ogg", loopStart = 1.059728, loopEnd = 2.577460 }
}, {
    volume = 0.35,
    releaseTime = 0.3
} )

MKeyboard:RegisterInstrument( "Music Box", "sound/musical_keyboard/instruments/music_box", {
{ note = 48, fileName = "C3.ogg" },
{ note = 60, fileName = "C4.ogg" },
{ note = 72, fileName = "C5.ogg" },
{ note = 84, fileName = "C6.ogg" },
{ note = 96, fileName = "C7.ogg" }
}, {
    volume = 0.3,
    releaseTime = 0.2
} )

MKeyboard:RegisterInstrument( "Harp", "sound/musical_keyboard/instruments/harp", {
{ note = 34, fileName = "A^1.ogg" },
{ note = 44, fileName = "G^2.ogg" },
{ note = 49, fileName = "C^3.ogg" },
{ note = 54, fileName = "F^3.ogg" },
{ note = 61, fileName = "C^4.ogg" },
{ note = 63, fileName = "D^4.ogg" },
{ note = 68, fileName = "G^4.ogg" },
{ note = 73, fileName = "C^5.ogg" },
{ note = 78, fileName = "F^5.ogg" },
{ note = 83, fileName = "B5.ogg" }
}, {
    volume = 0.4,
    releaseTime = 0.3
} )

MKeyboard:RegisterInstrument( "Harpsichord", "sound/musical_keyboard/instruments/harpsichord", {
{ note = 31, fileName = "G1.ogg" },
{ note = 33, fileName = "A1.ogg" },
{ note = 35, fileName = "B1.ogg" },
{ note = 36, fileName = "C2.ogg" },
{ note = 38, fileName = "D2.ogg" },
{ note = 40, fileName = "E2.ogg" },
{ note = 41, fileName = "F2.ogg" },
{ note = 43, fileName = "G2.ogg" },
{ note = 45, fileName = "A2.ogg" },
{ note = 47, fileName = "B2.ogg" },
{ note = 48, fileName = "C3.ogg" },
{ note = 50, fileName = "D3.ogg" },
{ note = 52, fileName = "E3.ogg" },
{ note = 53, fileName = "F3.ogg" },
{ note = 55, fileName = "G3.ogg" },
{ note = 57, fileName = "A3.ogg" },
{ note = 59, fileName = "B3.ogg" },
{ note = 60, fileName = "C4.ogg" },
{ note = 62, fileName = "D4.ogg" },
{ note = 64, fileName = "E4.ogg" },
{ note = 65, fileName = "F4.ogg" },
{ note = 67, fileName = "G4.ogg" },
{ note = 69, fileName = "A4.ogg" },
{ note = 71, fileName = "B4.ogg" },
{ note = 72, fileName = "C5.ogg" },
{ note = 74, fileName = "D5.ogg" },
{ note = 76, fileName = "E5.ogg" },
{ note = 77, fileName = "F5.ogg" },
{ note = 79, fileName = "G5.ogg" },
{ note = 80, fileName = "G^5.ogg" },
{ note = 81, fileName = "A5.ogg" },
{ note = 82, fileName = "A^5.ogg" },
{ note = 83, fileName = "B5.ogg" },
{ note = 84, fileName = "C6.ogg" },
{ note = 85, fileName = "C^6.ogg" }
}, {
    volume = 0.16,
    releaseTime = 0.3
} )

MKeyboard:RegisterInstrument( "Nylon Guitar", "sound/musical_keyboard/instruments/nylon_guitar", {
{ note = 40, fileName = "E2.ogg" },
{ note = 44, fileName = "G^2.ogg" },
{ note = 47, fileName = "B2.ogg" },
{ note = 51, fileName = "D^3.ogg" },
{ note = 55, fileName = "G3.ogg" },
{ note = 58, fileName = "A^3.ogg" },
{ note = 64, fileName = "E4.ogg" },
{ note = 66, fileName = "F^4.ogg" },
{ note = 69, fileName = "A4.ogg" },
{ note = 72, fileName = "C5.ogg" },
{ note = 79, fileName = "G5.ogg" }
}, {
    volume = 0.5,
    releaseTime = 0.2
} )

MKeyboard:RegisterInstrument( "Jazz Guitar", "sound/musical_keyboard/instruments/jazz_guitar", {
{ note = 40, fileName = "E2.ogg" },
{ note = 45, fileName = "A2.ogg" },
{ note = 50, fileName = "D3.ogg" },
{ note = 53, fileName = "F3.ogg" },
{ note = 58, fileName = "A^3.ogg" },
{ note = 61, fileName = "C^4.ogg" },
{ note = 64, fileName = "E4.ogg" },
{ note = 67, fileName = "G4.ogg" },
{ note = 70, fileName = "A^4.ogg" },
{ note = 73, fileName = "C^5.ogg" },
{ note = 77, fileName = "F5.ogg" },
{ note = 79, fileName = "G5.ogg" },
{ note = 82, fileName = "A^5.ogg" }
}, {
    volume = 0.5,
    releaseTime = 0.15
} )

MKeyboard:RegisterInstrument( "Picked Bass", "sound/musical_keyboard/instruments/picked_bass", {
{ note = 23, fileName = "B0.ogg", loopStart = 1.951429, loopEnd = 1.983900 },
{ note = 24, fileName = "C1.ogg", loopStart = 1.962971, loopEnd = 1.993628 },
{ note = 26, fileName = "D1.ogg", loopStart = 1.972381, loopEnd = 1.999637 },
{ note = 28, fileName = "E1.ogg", loopStart = 1.971814, loopEnd = 1.996100 },
{ note = 29, fileName = "F1.ogg", loopStart = 1.982268, loopEnd = 2.005215 },
{ note = 31, fileName = "G1.ogg", loopStart = 1.969796, loopEnd = 1.990295 },
{ note = 33, fileName = "A1.ogg", loopStart = 1.958141, loopEnd = 1.994490 },
{ note = 35, fileName = "B1.ogg", loopStart = 1.961383, loopEnd = 1.993764 },
{ note = 36, fileName = "C2.ogg", loopStart = 1.957415, loopEnd = 1.988005 },
{ note = 38, fileName = "D2.ogg", loopStart = 1.968526, loopEnd = 1.995782 },
{ note = 40, fileName = "E2.ogg", loopStart = 1.980385, loopEnd = 2.004649 },
{ note = 41, fileName = "F2.ogg", loopStart = 1.983560, loopEnd = 2.006485 },
{ note = 43, fileName = "G2.ogg", loopStart = 1.977800, loopEnd = 1.998209 },
{ note = 45, fileName = "A2.ogg", loopStart = 1.984830, loopEnd = 2.003016 },
{ note = 47, fileName = "B2.ogg", loopStart = 1.990159, loopEnd = 2.006372 },
{ note = 48, fileName = "C3.ogg", loopStart = 1.986984, loopEnd = 2.002290 },
{ note = 50, fileName = "D3.ogg", loopStart = 1.988367, loopEnd = 2.002018 },
{ note = 52, fileName = "E3.ogg", loopStart = 1.992925, loopEnd = 2.005079 },
{ note = 53, fileName = "F3.ogg", loopStart = 1.994807, loopEnd = 2.006236 },
{ note = 55, fileName = "G3.ogg", loopStart = 1.995374, loopEnd = 2.005578 },
{ note = 57, fileName = "A3.ogg", loopStart = 1.998594, loopEnd = 2.007687 },
{ note = 59, fileName = "B3.ogg", loopStart = 1.996077, loopEnd = 2.004172 },
{ note = 60, fileName = "C4.ogg", loopStart = 1.996281, loopEnd = 2.003946 },
{ note = 62, fileName = "D4.ogg", loopStart = 1.992132, loopEnd = 2.005782 },
{ note = 64, fileName = "E4.ogg", loopStart = 1.994717, loopEnd = 2.006848 },
{ note = 65, fileName = "F4.ogg", loopStart = 1.996689, loopEnd = 2.002404 }
}, {
    volume = 0.45,
    releaseTime = 0.15
} )

MKeyboard:RegisterInstrument( "Slap Bass", "sound/musical_keyboard/instruments/slap_bass", {
{ note = 28, fileName = "E1.ogg", loopStart = 6.076780, loopEnd = 6.101043 },
{ note = 29, fileName = "F1.ogg", loopStart = 5.304490, loopEnd = 5.327528 },
{ note = 33, fileName = "A1.ogg", loopStart = 6.345624, loopEnd = 6.363719 },
{ note = 35, fileName = "B1.ogg", loopStart = 1.042608, loopEnd = 1.407710 },
{ note = 38, fileName = "D2.ogg", loopStart = 5.598934, loopEnd = 5.967460 },
{ note = 43, fileName = "G2.ogg", loopStart = 5.833855, loopEnd = 5.844172 },
{ note = 47, fileName = "B2.ogg", loopStart = 2.633197, loopEnd = 2.641270 },
{ note = 50, fileName = "D3.ogg", loopStart = 3.432653, loopEnd = 3.439456 },
{ note = 53, fileName = "F3.ogg", loopStart = 4.220159, loopEnd = 4.225941 },
{ note = 56, fileName = "G^3.ogg", loopStart = 0.775941, loopEnd = 0.780771 }
}, {
    volume = 0.4,
    releaseTime = 0.15
} )

MKeyboard:RegisterInstrument( "Marimba", "sound/musical_keyboard/instruments/marimba", {
{ note = 41, fileName = "F2.ogg" },
{ note = 48, fileName = "C3.ogg" },
{ note = 55, fileName = "G3.ogg" },
{ note = 59, fileName = "B3.ogg" },
{ note = 65, fileName = "F4.ogg" },
{ note = 72, fileName = "C5.ogg" },
{ note = 79, fileName = "G5.ogg" },
{ note = 83, fileName = "B5.ogg" },
{ note = 89, fileName = "F6.ogg" },
{ note = 96, fileName = "C7.ogg" }
}, {
    volume = 0.3,
    releaseTime = 0.3
} )

MKeyboard:RegisterInstrument( "Vibraphone", "sound/musical_keyboard/instruments/vibraphone", {
{ note = 57, fileName = "A3.ogg" },
{ note = 62, fileName = "D4.ogg" },
{ note = 65, fileName = "F4.ogg" },
{ note = 69, fileName = "A4.ogg" },
{ note = 71, fileName = "B4.ogg" },
{ note = 76, fileName = "E5.ogg" },
{ note = 81, fileName = "A5.ogg" }
}, {
    volume = 0.3,
    releaseTime = 0.3
} )

MKeyboard:RegisterInstrument( "Flute", "sound/musical_keyboard/instruments/flute", {
    { note = 61, fileName = "C^4.ogg", loopStart = 0.659501, loopEnd = 0.911383 },
    { note = 64, fileName = "E4.ogg", loopStart = 0.725465, loopEnd = 0.975261 },
    { note = 67, fileName = "G4.ogg", loopStart = 0.644240, loopEnd = 0.870771 },
    { note = 71, fileName = "B4.ogg", loopStart = 0.380726, loopEnd = 0.615351 },
    { note = 74, fileName = "D5.ogg", loopStart = 1.015941, loopEnd = 1.253900 },
    { note = 78, fileName = "F^5.ogg", loopStart = 0.440703, loopEnd = 0.679184 },
    { note = 80, fileName = "G^5.ogg", loopStart = 0.255578, loopEnd = 0.499229 },
    { note = 84, fileName = "C6.ogg", loopStart = 0.510862, loopEnd = 0.737256 },
    { note = 90, fileName = "F^6.ogg", loopStart = 0.425556, loopEnd = 0.679184 }
}, {
    volume = 0.25,
    releaseTime = 0.1
} )

MKeyboard:RegisterInstrument( "Piccolo", "sound/musical_keyboard/instruments/piccolo", {
{ note = 76, fileName = "E5.ogg", loopStart = 0.519138, loopEnd = 0.75466 },
{ note = 78, fileName = "F^5.ogg", loopStart = 0.558980, loopEnd = 0.806893 },
{ note = 80, fileName = "G^5.ogg", loopStart = 0.716236, loopEnd = 0.980181 },
{ note = 82, fileName = "A^5.ogg", loopStart = 0.510884, loopEnd = 0.760454 },
{ note = 84, fileName = "C6.ogg", loopStart = 0.549546, loopEnd = 0.801088 },
{ note = 88, fileName = "E6.ogg", loopStart = 0.512268, loopEnd = 0.754649 },
{ note = 92, fileName = "G^6.ogg", loopStart = 0.664331, loopEnd = 0.905601 },
{ note = 96, fileName = "C7.ogg", loopStart = 0.646689, loopEnd = 0.888163 }
}, {
    volume = 0.25,
    releaseTime = 0.2
} )

MKeyboard:RegisterInstrument( "French Horn", "sound/musical_keyboard/instruments/french_horn", {
{ note = 34, fileName = "A^1.ogg", loopStart = 1.436137, loopEnd = 2.963280 },
{ note = 38, fileName = "D2.ogg", loopStart = 1.046296, loopEnd = 2.649723 },
{ note = 42, fileName = "F^2.ogg", loopStart = 0.892577, loopEnd = 2.618907 },
{ note = 46, fileName = "A^2.ogg", loopStart = 1.085568, loopEnd = 2.588064 },
{ note = 50, fileName = "D3.ogg", loopStart = 1.094784, loopEnd = 2.555296 },
{ note = 58, fileName = "A^3.ogg", loopStart = 1.027072, loopEnd = 2.488256 },
{ note = 62, fileName = "D4.ogg", loopStart = 1.238487, loopEnd = 2.757366 },
{ note = 74, fileName = "D5.ogg", loopStart = 1.706853, loopEnd = 3.027762 }
}, {
    volume = 0.3,
    releaseTime = 0.3
} )

MKeyboard:RegisterInstrument( "Strings", "sound/musical_keyboard/instruments/strings", {
{ note = 28, fileName = "E1.ogg", loopStart = 0.704354, loopEnd = 2.637143 },
{ note = 30, fileName = "F^1.ogg", loopStart = 0.986417, loopEnd = 2.922630 },
{ note = 33, fileName = "A1.ogg", loopStart = 0.658322, loopEnd = 2.711383 },
{ note = 36, fileName = "C2.ogg", loopStart = 0.606417, loopEnd = 2.984830 },
{ note = 39, fileName = "D^2.ogg", loopStart = 0.436803, loopEnd = 1.896032 },
{ note = 43, fileName = "G2.ogg", loopStart = 0.635556, loopEnd = 2.198209 },
{ note = 46, fileName = "A^2.ogg", loopStart = 0.694331, loopEnd = 2.918934 },
{ note = 48, fileName = "C3.ogg", loopStart = 0.512948, loopEnd = 2.511451 },
{ note = 52, fileName = "E3.ogg", loopStart = 0.425510, loopEnd = 2.167528 },
{ note = 55, fileName = "G3.ogg", loopStart = 0.601927, loopEnd = 2.471791 },
{ note = 59, fileName = "B3.ogg", loopStart = 0.645918, loopEnd = 2.967029 },
{ note = 62, fileName = "D4.ogg", loopStart = 0.706304, loopEnd = 2.478231 },
{ note = 65, fileName = "F4.ogg", loopStart = 0.395465, loopEnd = 2.124626 },
{ note = 68, fileName = "G^4.ogg", loopStart = 0.551361, loopEnd = 2.351859 },
{ note = 70, fileName = "A^4.ogg", loopStart = 0.440249, loopEnd = 2.093878 },
{ note = 75, fileName = "D^5.ogg", loopStart = 0.378889, loopEnd = 2.255465 },
{ note = 79, fileName = "G5.ogg", loopStart = 0.254399, loopEnd = 1.610590 },
{ note = 82, fileName = "A^5.ogg", loopStart = 0.379070, loopEnd = 1.777642 },
{ note = 88, fileName = "E6.ogg", loopStart = 0.364467, loopEnd = 1.851020 },
{ note = 91, fileName = "G6.ogg", loopStart = 0.506916, loopEnd = 1.957937 },
{ note = 93, fileName = "A6.ogg", loopStart = 0.335964, loopEnd = 1.659206 }
}, {
    volume = 0.3,
    releaseTime = 0.2
} )

MKeyboard:RegisterInstrument( "Pizzicato Strings", "sound/musical_keyboard/instruments/pizzicato_strings", {
{ note = 36, fileName = "C2.ogg" },
{ note = 43, fileName = "G2.ogg" },
{ note = 48, fileName = "C3.ogg" },
{ note = 55, fileName = "G3.ogg" },
{ note = 60, fileName = "C4.ogg" },
{ note = 67, fileName = "G4.ogg" },
{ note = 72, fileName = "C5.ogg" },
{ note = 79, fileName = "G5.ogg" },
{ note = 84, fileName = "C6.ogg" }
}, {
    volume = 0.4,
    releaseTime = 0.3
} )

MKeyboard:RegisterInstrument( "Choir Aahs", "sound/musical_keyboard/instruments/choir_aahs", {
    { note = 39, fileName = "D^2.ogg", loopStart = 1.209750, loopEnd = 2.061156 },
    { note = 42, fileName = "F^2.ogg", loopStart = 0.672531, loopEnd = 2.282875 },
    { note = 45, fileName = "A2.ogg", loopStart = 0.906000, loopEnd = 2.782812 },
    { note = 48, fileName = "C3.ogg", loopStart = 1.424000, loopEnd = 3.015000 },
    { note = 51, fileName = "D^3.ogg", loopStart = 1.377406, loopEnd = 3.146000 },
    { note = 54, fileName = "F^3.ogg", loopStart = 1.068688, loopEnd = 3.034375 },
    { note = 57, fileName = "A3.ogg", loopStart = 0.914312, loopEnd = 2.608875 },
    { note = 60, fileName = "C4.ogg", loopStart = 1.301688, loopEnd = 2.557719 },
    { note = 63, fileName = "D^4.ogg", loopStart = 1.052000, loopEnd = 2.460469 },
    { note = 66, fileName = "F^4.ogg", loopStart = 0.560969, loopEnd = 2.750875 },
    { note = 69, fileName = "A4.ogg", loopStart = 0.656969, loopEnd = 2.424594 },
    { note = 72, fileName = "C5.ogg", loopStart = 0.562250, loopEnd = 1.660187 },
    { note = 75, fileName = "D^5.ogg", loopStart = 0.734000, loopEnd = 2.293594 },
    { note = 78, fileName = "F^5.ogg", loopStart = 0.468813, loopEnd = 1.725281 },
    { note = 81, fileName = "A5.ogg", loopStart = 0.831469, loopEnd = 2.320781 },
}, {
    volume = 0.4,
    releaseTime = 0.6
} )

local function AddInstrumentWithNoteRange( name, basePath, startNote, endNote, extension, params )
    local samples = {}

    for note = startNote, endNote do
        samples[#samples + 1] = { note = note, fileName = note .. extension }
    end

    MKeyboard:RegisterInstrument( name, basePath, samples, params )
end

AddInstrumentWithNoteRange( "Standard Drums", "sound/musical_keyboard/instruments/drums_standard", 26, 87, ".mp3", {
    volume = 0.3,
    releaseTime = 0.5
} )

MKeyboard:RegisterInstrument( "Accordion", "sound/musical_keyboard/instruments/accordion", {
{ note = 53, fileName = "F3.ogg", loopStart = 0.496168, loopEnd = 0.999456 },
{ note = 55, fileName = "G3.ogg", loopStart = 0.421202, loopEnd = 0.839274 },
{ note = 57, fileName = "A3.ogg", loopStart = 0.575306, loopEnd = 0.957120 },
{ note = 59, fileName = "B3.ogg", loopStart = 0.621156, loopEnd = 0.945351 },
{ note = 64, fileName = "E4.ogg", loopStart = 0.559070, loopEnd = 0.832404 },
{ note = 65, fileName = "F4.ogg", loopStart = 0.819002, loopEnd = 1.094218 },
{ note = 67, fileName = "G4.ogg", loopStart = 0.661587, loopEnd = 0.888889 },
{ note = 69, fileName = "A4.ogg", loopStart = 0.306145, loopEnd = 0.542766 },
{ note = 71, fileName = "B4.ogg", loopStart = 0.334422, loopEnd = 0.547415 },
{ note = 72, fileName = "C5.ogg", loopStart = 0.311973, loopEnd = 0.554535 },
{ note = 74, fileName = "D5.ogg", loopStart = 0.403401, loopEnd = 0.643469 },
{ note = 77, fileName = "F5.ogg", loopStart = 0.528866, loopEnd = 0.714649 },
{ note = 79, fileName = "G5.ogg", loopStart = 0.243628, loopEnd = 0.372404 },
{ note = 82, fileName = "A^5.ogg", loopStart = 0.318844, loopEnd = 0.511383 },
{ note = 83, fileName = "B5.ogg", loopStart = 0.165805, loopEnd = 0.325669 },
{ note = 84, fileName = "C6.ogg", loopStart = 0.373175, loopEnd = 0.532653 },
{ note = 88, fileName = "E6.ogg", loopStart = 0.336984, loopEnd = 0.490952 }
}, {
    volume = 0.4,
    releaseTime = 0.05
} )

MKeyboard:RegisterInstrument( "Soprano Sax", "sound/musical_keyboard/instruments/soprano_sax", {
{ note = 56, fileName = "G^3.ogg", loopStart = 1.461519, loopEnd = 2.179864 },
{ note = 59, fileName = "B3.ogg", loopStart = 2.069161, loopEnd = 2.830680 },
{ note = 63, fileName = "D^4.ogg", loopStart = 1.611927, loopEnd = 2.168141 },
{ note = 67, fileName = "G4.ogg", loopStart = 2.061111, loopEnd = 2.706168 },
{ note = 70, fileName = "A^4.ogg", loopStart = 1.919819, loopEnd = 2.615420 },
{ note = 73, fileName = "C^5.ogg", loopStart = 1.900544, loopEnd = 2.413673 },
{ note = 76, fileName = "E5.ogg", loopStart = 2.274331, loopEnd = 2.819116 },
{ note = 81, fileName = "A5.ogg", loopStart = 2.632608, loopEnd = 3.008141 },
{ note = 88, fileName = "E6.ogg", loopStart = 2.236190, loopEnd = 2.705261 },
{ note = 91, fileName = "G6.ogg", loopStart = 1.451066, loopEnd = 2.279025 }
}, {
    volume = 0.35,
    releaseTime = 0.3
} )

MKeyboard:RegisterInstrument( "Brass Section", "sound/musical_keyboard/instruments/brass_section", {
{ note = 28, fileName = "E1.ogg", loopStart = 2.266961, loopEnd = 6.008458 },
{ note = 52, fileName = "E3.ogg", loopStart = 2.266984, loopEnd = 6.008481 },
{ note = 60, fileName = "C4.ogg", loopStart = 2.266961, loopEnd = 6.008458 },
{ note = 67, fileName = "G4.ogg", loopStart = 2.266893, loopEnd = 6.008390 },
{ note = 76, fileName = "E5.ogg", loopStart = 2.266939, loopEnd = 6.008435 },
{ note = 84, fileName = "C6.ogg", loopStart = 2.266939, loopEnd = 6.008435 }
}, {
    volume = 0.3,
    releaseTime = 0.1
} )

MKeyboard:RegisterInstrument( "Steel Guitar", "sound/musical_keyboard/instruments/steel_guitar", {
{ note = 40, fileName = "E2.ogg" },
{ note = 45, fileName = "A2.ogg" },
{ note = 51, fileName = "D^3.ogg" },
{ note = 60, fileName = "C4.ogg" },
{ note = 63, fileName = "D^4.ogg" },
{ note = 66, fileName = "F^4.ogg" },
{ note = 69, fileName = "A4.ogg" },
{ note = 72, fileName = "C5.ogg" },
{ note = 75, fileName = "D^5.ogg" },
{ note = 78, fileName = "F^5.ogg" },
{ note = 81, fileName = "A5.ogg" },
{ note = 84, fileName = "C6.ogg" }
}, {
    volume = 0.17,
    releaseTime = 0.3
} )

MKeyboard:RegisterInstrument( "Fretless Bass", "sound/musical_keyboard/instruments/fretless_bass", {
{ note = 29, fileName = "F1.ogg", loopStart = 1.196009, loopEnd = 1.218934 },
{ note = 32, fileName = "G^1.ogg", loopStart = 1.512540, loopEnd = 1.531882 },
{ note = 35, fileName = "B1.ogg", loopStart = 1.542562, loopEnd = 1.558776 },
{ note = 38, fileName = "D2.ogg", loopStart = 1.775193, loopEnd = 1.802472 },
{ note = 41, fileName = "F2.ogg", loopStart = 1.539274, loopEnd = 1.573651 },
{ note = 44, fileName = "G^2.ogg", loopStart = 1.449637, loopEnd = 1.478571 },
{ note = 47, fileName = "B2.ogg", loopStart = 1.625351, loopEnd = 1.657619 },
{ note = 50, fileName = "D3.ogg", loopStart = 1.410454, loopEnd = 1.437732 },
{ note = 53, fileName = "F3.ogg", loopStart = 1.542630, loopEnd = 1.565556 },
{ note = 59, fileName = "B3.ogg", loopStart = 1.759955, loopEnd = 1.784172 }
}, {
    volume = 0.35,
    releaseTime = 0.15
} )

MKeyboard:RegisterInstrument( "Jazz Organ", "sound/musical_keyboard/instruments/jazz_organ", {
{ note = 55, fileName = "G3.ogg", loopStart = 1.252562, loopEnd = 2.833424 },
{ note = 60, fileName = "C4.ogg", loopStart = 1.188912, loopEnd = 2.687596 },
{ note = 67, fileName = "G4.ogg", loopStart = 0.512971, loopEnd = 1.992562 },
{ note = 72, fileName = "C5.ogg", loopStart = 0.857914, loopEnd = 2.207302 },
{ note = 79, fileName = "G5.ogg", loopStart = 0.729138, loopEnd = 2.190862 },
{ note = 84, fileName = "C6.ogg", loopStart = 0.444558, loopEnd = 1.983379 },
{ note = 91, fileName = "G6.ogg", loopStart = 0.354830, loopEnd = 2.331882 },
{ note = 96, fileName = "C7.ogg", loopStart = 0.256893, loopEnd = 2.003265 }
}, {
    volume = 0.3,
    releaseTime = 0.15
} )

MKeyboard:RegisterInstrument( "Violin", "sound/musical_keyboard/instruments/violin", {
{ note = 55, fileName = "G3.ogg", loopStart = 0.958118, loopEnd = 2.737891 },
{ note = 57, fileName = "A3.ogg", loopStart = 1.617347, loopEnd = 2.625510 },
{ note = 58, fileName = "A^3.ogg", loopStart = 1.657710, loopEnd = 2.470136 },
{ note = 59, fileName = "B3.ogg", loopStart = 1.754898, loopEnd = 2.272517 },
{ note = 61, fileName = "C^4.ogg", loopStart = 2.669206, loopEnd = 3.015170 },
{ note = 63, fileName = "D^4.ogg", loopStart = 2.645465, loopEnd = 2.981927 },
{ note = 64, fileName = "E4.ogg", loopStart = 2.522789, loopEnd = 2.689229 },
{ note = 65, fileName = "F4.ogg", loopStart = 1.883537, loopEnd = 2.055488 },
{ note = 66, fileName = "F^4.ogg", loopStart = 2.199705, loopEnd = 2.372744 },
{ note = 68, fileName = "G^4.ogg", loopStart = 1.806621, loopEnd = 2.139796 },
{ note = 70, fileName = "A^4.ogg", loopStart = 2.479478, loopEnd = 2.652902 },
{ note = 71, fileName = "B4.ogg", loopStart = 1.850091, loopEnd = 2.022404 },
{ note = 73, fileName = "C^5.ogg", loopStart = 2.079138, loopEnd = 2.253583 },
{ note = 75, fileName = "D^5.ogg", loopStart = 1.652857, loopEnd = 1.832245 },
{ note = 77, fileName = "F5.ogg", loopStart = 2.167664, loopEnd = 2.355170 },
{ note = 79, fileName = "G5.ogg", loopStart = 2.607937, loopEnd = 2.989138 },
{ note = 80, fileName = "G^5.ogg", loopStart = 1.900295, loopEnd = 2.594014 },
{ note = 82, fileName = "A^5.ogg", loopStart = 1.752200, loopEnd = 2.400522 },
{ note = 83, fileName = "B5.ogg", loopStart = 1.589705, loopEnd = 1.779796 },
{ note = 84, fileName = "C6.ogg", loopStart = 2.146054, loopEnd = 2.816213 },
{ note = 87, fileName = "D^6.ogg", loopStart = 1.567937, loopEnd = 2.587687 },
{ note = 90, fileName = "F^6.ogg", loopStart = 1.111066, loopEnd = 2.428798 },
{ note = 91, fileName = "G6.ogg", loopStart = 0.804354, loopEnd = 2.022132 },
{ note = 94, fileName = "A^6.ogg", loopStart = 1.068435, loopEnd = 2.335261 },
{ note = 95, fileName = "B6.ogg", loopStart = 1.072426, loopEnd = 2.609615 }
}, {
    volume = 0.3,
    releaseTime = 0.15
} )

MKeyboard:RegisterInstrument( "Church Organ", "sound/musical_keyboard/instruments/church_organ", {
{ note = 24, fileName = "C0.ogg", loopStart = 4.367120, loopEnd = 12.911474 },
{ note = 25, fileName = "C^0.ogg", loopStart = 4.237370, loopEnd = 12.527914 },
{ note = 26, fileName = "D0.ogg", loopStart = 4.195215, loopEnd = 12.403243 },
{ note = 27, fileName = "D^0.ogg", loopStart = 4.360522, loopEnd = 12.891995 },
{ note = 28, fileName = "E0.ogg", loopStart = 4.302426, loopEnd = 12.720227 },
{ note = 29, fileName = "F0.ogg", loopStart = 4.285896, loopEnd = 12.671361 },
{ note = 30, fileName = "F^0.ogg", loopStart = 4.461859, loopEnd = 13.191633 },
{ note = 31, fileName = "G0.ogg", loopStart = 4.271633, loopEnd = 12.629206 },
{ note = 32, fileName = "G^0.ogg", loopStart = 4.240249, loopEnd = 12.536417 },
{ note = 33, fileName = "A0.ogg", loopStart = 4.262177, loopEnd = 12.601247 },
{ note = 34, fileName = "A^0.ogg", loopStart = 4.002698, loopEnd = 11.834104 },
{ note = 35, fileName = "B0.ogg", loopStart = 4.294036, loopEnd = 12.695442 },
{ note = 36, fileName = "C1.ogg", loopStart = 6.358889, loopEnd = 18.800227 },
{ note = 37, fileName = "C^1.ogg", loopStart = 4.293333, loopEnd = 12.693333 },
{ note = 38, fileName = "D1.ogg", loopStart = 4.252472, loopEnd = 12.572540 },
{ note = 39, fileName = "D^1.ogg", loopStart = 4.287211, loopEnd = 12.675261 },
{ note = 40, fileName = "E1.ogg", loopStart = 4.148367, loopEnd = 12.264762 },
{ note = 41, fileName = "F1.ogg", loopStart = 4.475646, loopEnd = 13.232358 },
{ note = 42, fileName = "F^1.ogg", loopStart = 4.289252, loopEnd = 12.681270 },
{ note = 43, fileName = "G1.ogg", loopStart = 4.267914, loopEnd = 12.618231 },
{ note = 44, fileName = "G^1.ogg", loopStart = 4.154014, loopEnd = 12.281429 },
{ note = 45, fileName = "A1.ogg", loopStart = 4.271406, loopEnd = 12.628503 },
{ note = 46, fileName = "A^1.ogg", loopStart = 4.303152, loopEnd = 12.722358 },
{ note = 47, fileName = "B1.ogg", loopStart = 4.277755, loopEnd = 12.647279 },
{ note = 48, fileName = "C2.ogg", loopStart = 4.196168, loopEnd = 12.406077 },
{ note = 49, fileName = "C^2.ogg", loopStart = 4.239410, loopEnd = 12.533946 },
{ note = 50, fileName = "D2.ogg", loopStart = 4.223832, loopEnd = 12.487914 },
{ note = 51, fileName = "D^2.ogg", loopStart = 4.052902, loopEnd = 11.982494 },
{ note = 52, fileName = "E2.ogg", loopStart = 4.274626, loopEnd = 12.638050 },
{ note = 53, fileName = "F2.ogg", loopStart = 4.144898, loopEnd = 12.254490 },
{ note = 54, fileName = "F^2.ogg", loopStart = 4.105850, loopEnd = 12.139048 },
{ note = 55, fileName = "G2.ogg", loopStart = 4.264444, loopEnd = 12.607959 },
{ note = 56, fileName = "G^2.ogg", loopStart = 4.318481, loopEnd = 12.767687 },
{ note = 57, fileName = "A2.ogg", loopStart = 4.251383, loopEnd = 12.569342 },
{ note = 58, fileName = "A^2.ogg", loopStart = 4.242766, loopEnd = 12.543855 },
{ note = 59, fileName = "B2.ogg", loopStart = 4.288526, loopEnd = 12.679161 },
{ note = 60, fileName = "C3.ogg", loopStart = 4.291156, loopEnd = 12.686939 },
{ note = 61, fileName = "C^3.ogg", loopStart = 4.363878, loopEnd = 12.901927 },
{ note = 62, fileName = "D3.ogg", loopStart = 4.053016, loopEnd = 11.982857 },
{ note = 63, fileName = "D^3.ogg", loopStart = 4.164172, loopEnd = 12.311519 },
{ note = 64, fileName = "E3.ogg", loopStart = 4.117234, loopEnd = 12.172676 },
{ note = 65, fileName = "F3.ogg", loopStart = 4.245170, loopEnd = 12.550930 },
{ note = 66, fileName = "F^3.ogg", loopStart = 4.212698, loopEnd = 12.454966 },
{ note = 67, fileName = "G3.ogg", loopStart = 4.207800, loopEnd = 12.440454 },
{ note = 68, fileName = "G^3.ogg", loopStart = 4.086667, loopEnd = 12.082381 },
{ note = 69, fileName = "A3.ogg", loopStart = 4.351655, loopEnd = 12.865782 },
{ note = 70, fileName = "A^3.ogg", loopStart = 4.290680, loopEnd = 12.685510 },
{ note = 71, fileName = "B3.ogg", loopStart = 4.346032, loopEnd = 12.849161 },
{ note = 72, fileName = "C4.ogg", loopStart = 4.237370, loopEnd = 12.527914 },
{ note = 73, fileName = "C^4.ogg", loopStart = 4.183719, loopEnd = 12.369252 },
{ note = 74, fileName = "D4.ogg", loopStart = 4.283379, loopEnd = 12.663923 },
{ note = 75, fileName = "D^4.ogg", loopStart = 4.207438, loopEnd = 12.439388 },
{ note = 76, fileName = "E4.ogg", loopStart = 4.145850, loopEnd = 12.257347 },
{ note = 77, fileName = "F4.ogg", loopStart = 4.208503, loopEnd = 12.442562 },
{ note = 78, fileName = "F^4.ogg", loopStart = 4.197732, loopEnd = 12.410680 },
{ note = 79, fileName = "G4.ogg", loopStart = 4.125374, loopEnd = 12.196757 },
{ note = 80, fileName = "G^4.ogg", loopStart = 8.861927, loopEnd = 26.200522 },
{ note = 81, fileName = "A4.ogg", loopStart = 4.114104, loopEnd = 12.163469 },
{ note = 82, fileName = "A^4.ogg", loopStart = 4.231020, loopEnd = 12.509161 },
{ note = 83, fileName = "B4.ogg", loopStart = 3.923039, loopEnd = 11.598594 },
{ note = 84, fileName = "C5.ogg", loopStart = 4.408685, loopEnd = 13.034376 }
}, {
    volume = 0.4,
    releaseTime = 0.3
} )
